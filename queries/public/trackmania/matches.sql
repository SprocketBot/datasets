SELECT
  ...
  home.title AS home_franchise_name,
  sf."awayFranchiseId" AS away_franchise_id,
  away.title AS away_franchise_name,
  sg.id AS schedule_group_id,
  sg.description AS schedule_group_name,
...
FROM
  ...
  LEFT JOIN sprocket.match m ON m."matchParentId" = mp.id
  LEFT JOIN sprocket.schedule_fixture sf ON sf.id = mp."fixtureId"
  LEFT JOIN sprocket.franchise_profile home ON home.id = sf."homeFranchiseId"
  LEFT JOIN sprocket.franchise_profile away ON away.id = sf."awayFranchiseId"
  LEFT JOIN sprocket.schedule_group sg ON sg.id = sf."scheduleGroupId"
  LEFT JOIN sprocket.game_skill_group gsg ON gsg.id = m."skillGroupId"
  LEFT JOIN sprocket.game g ON g.id = gsg."gameId"
  LEFT JOIN sprocket.game_skill_group_profile gsgp ON gsgp."skillGroupId" = gsg.id
  LEFT JOIN sprocket.game_mode gm ON gm.id = m."gameModeId"
WHERE
  g.title = 'Trackmania'
