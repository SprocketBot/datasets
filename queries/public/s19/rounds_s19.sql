...
from
  ...
  inner join sprocket.team_stat_line tslh on (
    tslh."roundId" = r.id
    and tslh."teamName" = fph.title
  )
  inner join sprocket.team_stat_line tsla on (
    tsla."roundId" = r.id
    and tsla."teamName" = fpa.title
  )
where
  sg."parentGroupId" = 340
