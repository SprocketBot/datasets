WITH mle_team_meta AS (SELECT team.name, division_name, conference
                       FROM mledb.team
                                INNER JOIN mledb.division ON team.division_name = division.name
                       WHERE division_name != 'Meta'),
     raw_standings_data AS (SELECT CASE
                                       WHEN match_invalidation.id IS NOT NULL THEN
                                           ...
                                           CASE WHEN match_invalidation."favorsHomeTeam" THEN 5 ELSE 0 END
                                       ELSE
                                           SUM(
                                                   CASE
                                                       WHEN r."homeWon" IS NOT NULL THEN
                                                           CASE
                                                               WHEN round_invalidation IS NOT NULL THEN
                                                                   ...
                                                                   CASE WHEN round_invalidation."favorsHomeTeam" THEN 1 ELSE 0 END
                                                               -- Add 1 to result if home won
                                                               ELSE CASE WHEN r."homeWon" THEN 1 ELSE 0 END
                                                               END
                                                       ELSE 0
                                                       END
                                           ) END      as homeWins,
                                   ...
                                   CASE
                                       WHEN match_invalidation.id IS NOT NULL THEN
                                           CASE WHEN match_invalidation."favorsHomeTeam" THEN 0 ELSE 5 END
                                       ELSE SUM(
                                               CASE
                                                   WHEN r."homeWon" IS NOT NULL THEN
                                                       CASE
                                                           WHEN round_invalidation IS NOT NULL THEN
                                                               CASE WHEN round_invalidation."favorsHomeTeam" THEN 0 ELSE 1 END
                                                           ELSE CASE WHEN r."homeWon" THEN 0 ELSE 1 END
                                                           END
                                                   ELSE 0
                                                   END
                                            ) END     as awayWins,
                            ...
                            FROM sprocket.match m
                                     ...
                                     INNER JOIN sprocket.schedule_group season ON sg."parentGroupId" = season.id AND season."typeId" IN (SELECT id FROM sprocket.schedule_group_type WHERE code = 'SEASON')
